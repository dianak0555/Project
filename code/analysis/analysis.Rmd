...
```{r}
#Below is a correlation heatmap which shows how the different voice features relate to each other. This can also provide context for our result, as it suggests that if some features are strongly correlated, parkinson's disease could affect multiple vocal features in related ways. The color gradient helps read the graphs better in positive vs negative correlation.

parkinsons %>%
  correlate() %>%
  rplot(colors = c("red","white","blue")) +
  labs(title="Correlation Between Voice Features")
```
